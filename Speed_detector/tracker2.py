...
file = open("G://ABC//SpeedRecord.txt", "w")
file.write("ID \t SPEED\n------\t-------\n")
file.close()
...
class EuclideanDistTracker:
    ...
    def capture(self,img,x,y,h,w,sp,id):
        if(self.capf[id]==0):
            ...
            file = 'G://ABC//traffic' + n + '.jpg'
            cv2.imwrite(file, crop_img)
            self.count += 1
            filet = open("G://ABC//SpeedRecord.txt", "a")
            if(sp>limit):
                file2 = 'G://ABC//exceeded//' + n + '.jpg'
    ...
    #TEXT FILE SUMMARY
    def end(self):
        file = open("G://ABC//SpeedRecord.txt", "a")
        file.write("\n-------------\n")
        file.write("-------------\n")
        file.write("SUMMARY\n")
        file.write("-------------\n")
